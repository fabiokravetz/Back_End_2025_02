...
Autorização (O que você pode fazer?): O processo de verificar se um usuário 
autenticado tem permissão para acessar um recurso específico.
...
INSERIR ESSE CÓDIGO NO BD - Coluna criacao inserida automaticamente no BD
...
*/
const fatorHashSenha = 10; // Quando maior o valor mais demorado é o  
                          //  processo de hashing - Maior segurança.
...
function autenticacaoAdmin(req, res, next) {
    if (req.session.funcao === 'admin') { 
        ...
        res.status(403).send("Acesso negado. Apenas administradores podem executar esta ação.");
    }
}
...
app.post('/login', async (req, res) => {
    const { username, password } = req.body;// Diretamente relacionado ao urlencoded
    ...
    try {
        ...
        const [usuarios] = await pool.query(
            'SELECT id, username, password_hash, funcao FROM users WHERE username = ?', 
            [username]
        );
        ...
        if (verificaSenha) {//Se verdadeiro cria a sessão (Armazena ID, username e funcao)
            req.session.userId = usuario.id;
            req.session.username = usuario.username;
            req.session.funcao = usuario.funcao; // Armazena user ou admin
...
/*
Caso autenticacaoLogin chame next() - linha 97 -, a função final é executada, renderizando o 
paginaProtegida com o nome de usuário salvo na sessão.
*/
...
app.post('/paginaProtegida/inserir', autenticacaoLogin, async (req, res) => {
    const { nome, descricao } = req.body; // Diretamente relacionado ao urlencoded
    const usuario_id = req.session.userId; // Pega o ID da sessão

    if (!nome || !descricao) {
        return res.status(400).send("O nome do item e a descrição são obrigatórios!");
    }
...
// -------------------------------DELETE - ADMIN----------------------------
app.post('/paginaProtegida/deletar/:id', autenticacaoLogin, autenticacaoAdmin, async (req, res) => {
    ...
    try {
        ...
        console.log(`ID ${itemId} deletado pelo Administrador: ${req.session.username}`);
        res.redirect('/paginaProtegida'); 
...
app.get('/paginaProtegida/editar/:id', autenticacaoLogin, autenticacaoAdmin, async (req, res) => {
    const itemId = req.params.id;

    try {
        const [items] = await pool.query(
            'SELECT id, nome, descricao FROM items WHERE id = ?',
            [itemId]
        );
...
// ------------------------------------UPDATE de dados - ADMIN-------------------------------------
app.post('/paginaProtegida/update/:id', autenticacaoLogin, autenticacaoAdmin, async (req, res) => {
    const itemId = req.params.id;
    const { nome, descricao } = req.body; 

    if (!nome || !descricao) {
        return res.status(400).send("O nome e a descrição são obrigatórios.");
    }

    try {
        const [resultado] = await pool.query(
            'UPDATE items SET nome = ?, descricao = ? WHERE id = ?',
            [nome, descricao, itemId]
        );
        ...
        console.log(`Item ID ${itemId} atualizado por Admin: ${req.session.username}`);
        res.redirect('/paginaProtegida'); 
